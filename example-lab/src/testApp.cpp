...
void testApp::setup(){
    ...
    ofSetWindowShape(width*1.5, height);
    ...
    sandboxMirror.allocate(width,height, GL_RGB);
    
    grayScott.allocate(width,height);
...
void testApp::draw(){
    ...
    if ( selection == 0 ){
        ...
        blur.draw();
        title += " width blur filter (" + ofToString(sin(beat)*10,1) + " )";
        ofDrawBitmapString("ofxBlur.setRadius( " + ofToString(sin(beat)*10,1) + " )", 15, 15);
    } else if ( selection == 2 ){
        bokeh.draw();
        ...
        unsharp.draw();
        title += " width unsharp filter ( "+ ofToString(sin(beat),1)+ " )";
        ofDrawBitmapString("ofxUnsharp.setFade( " + ofToString(sin(beat),1) + " )", 15, 15);
    } else if ( selection == 5 ){
        oldtv.draw();
    ...
    sandboxMirror.draw(width,0,width*0.5,height*0.5);
    grayScott.draw(width,height*0.5,width*0.5,height*0.5);
    ...
    image.draw(width *0.5 - image.getWidth()*0.5, height*0.5 - image.getHeight()*0.5);
    ofEnableBlendMode(OF_BLENDMODE_ALPHA);
    ofPopStyle();
}
...
void testApp::keyPressed(int key){
    switch(key){
        ...
        case OF_KEY_LEFT:
            ...
            break;
        case OF_KEY_RETURN:
            sandboxMirror = sandbox;
            break;
        case ' ':
            sandboxMirror >> grayScott;
            //grayScott[0] = sandbox.getTextureReference();
            break;
    }
